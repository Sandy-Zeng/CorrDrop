...
from dropblock.corrdrop import CorrDrop2D
...
class SCDBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, drop_rate):
        super(SCDBlock, self).__init__()
        ...
        if in_channels != out_channels:
            self.shortcut.add_module(
                ...
                nn.Conv2d(
                    ...
                    padding=0,
                    bias=False))
        self.corrdrop_block = CorrDropBlockDB(drop_prob=0.2, block_size=5)
    ...
    def forward(self, x):
        ...
        y = F.relu(self.bn2(y), inplace=True)
        y = self.conv2(y)
        y = self.corrdrop_block(y)
        y += self.shortcut(x)
        return y


class CCDBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, drop_rate):
        super(CCDBlock, self).__init__()
...
class Network(nn.Module):
    def __init__(self, config):
        ...
        if self.dropway == 'scd':
            stage_block = SCDBlock
        elif self.dropway == 'ccd':
            stage_block = CCDBlock
        elif self.dropway == 'dropblock':
            stage_block = DBBlock
        else:
            stage_block = BasicBlock
